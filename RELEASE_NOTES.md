...
Extract **SuperOpti-windows-x64.zip**, then run portable or install for the current user. This unsigned preview retains explicit limitations: observed process-core attribution and thread-to-file activity are unavailable; live CPU/GPU throttle overlays and full IPv6 scope/connection-instance validation remain unfinished. Captured traffic totals describe observed ETW payload events and can be incomplete. See README and VALIDATION for tested scope and remaining checks.
